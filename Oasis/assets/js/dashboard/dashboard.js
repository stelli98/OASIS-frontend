...
$(document).ready(function () {
    ...
    function loadNewAssetRequest() {
        $.ajax({
            type: 'GET',
            url: path + '/api/dashboard/request-update/' + username + '?tab=others&page=1&sort=D-updatedDate',
            contentType: 'application/octet-stream',
            dataType: 'json',
            success: function (data) {
                if (data.code == statusSuccess) {
                    ...
                    for (index; index < minimumAssetTotal; index++) {
                        ...
                        <div class='table-content table-content-new-request-admin-name'  id='table-content-new-request-admin-name--${index}'>${data.value.requests[index].employee.username}</div>
                        <div class='table-content table-content-new-request-admin-asset' id='table-content-new-request-admin-asset--${index}'>${data.value.requests[index].asset.name}</div>
                        <div class='table-content table-content-new-request-admin-qty'   id='table-content-new-request-admin-qty--${index}'>${data.value.requests[index].asset.quantity}</div>
                        <div class='table-content table-content-new-request-admin-note'  id='table-content-new-request-admin-note--${index}'>${data.value.requests[index].request.note}</div>
